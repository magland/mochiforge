...
`network.trustProxy` and the rest of the `limits` block are readable and not writable. They are read once when the server starts (see [Deploying a vault](deploying.md)), so a route that changed them would report a change the running server had not made. Edit `config.json` in the vault and restart, or use the vault settings page in the web interface (`/admin/settings`), which writes the same file and says which saved values the running server is not yet using.
