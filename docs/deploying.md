...
Not every repository is eligible for a hostname of its own, because not every legal repository name is a legal DNS label; an ineligible one keeps being served on the forge host under the sandbox, or its admin picks a custom label that is one. [Sites](sites.md) gives the rule, describes the per-repository settings (a site is opt-in, and each repository may choose its label under the sites host), and describes what a per-site origin does and does not isolate.
