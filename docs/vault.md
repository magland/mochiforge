...
Four conveniences sit beside the token form, all ending in the same session cookie and all revocable from `vault.json` like everything else.
...
**Sign in with GitHub.** An administrator can let GitHub accounts sign in: create a GitHub OAuth App whose authorization callback URL is `https://<vault-host>/login/github/callback`, and enter its client id and client secret on `/admin/github`. The client id is public and lives in `config.json` (`auth.githubClientId`); the secret is written to `<vault>/.github-secret` beside `.secret`, mode 600. Two kinds of GitHub account may then sign in. A user who has linked their GitHub account from their own account page signs in as themselves; and a GitHub account on the approved list (managed on `/admin/github`, entered by username and recorded by GitHub's stable numeric id, so a later rename on GitHub transfers nothing) is given an account on its first sign-in, named after its GitHub login and holding no tokens until an administrator mints one, so such a user can use the web but not push until then. The vault stores only the GitHub account's numeric id and login, as a `github` field on the user in `vault.json`; the OAuth access token is used for the single "who is this" request and dropped, so the vault holds no GitHub token. A session signed in this way has the user's full standing, like a passkey session, and resolves the link against live `vault.json` on every request, so unlinking (on the account page, or by an administrator on `/admin/users/<name>`) ends its sessions at once. Removing an entry from the approved list stops new accounts being created but does not touch an account already linked; unlinking is the off switch for those. git does not speak OAuth, so pushing keeps using tokens.
